...
export const getLockers = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: 'Error al obtener los lockers',
      error: error.message
    });
  }
};
export const openVirtualLocker = async (req, res) => {
  ...
  try {
    await sequelize.query('CALL INS_REL_LOCKER_CUSTOMER(:cod_customer, :cod_locker, @p_locker_code, @p_success, @p_message)', {
      replacements: {
        cod_customer,
        cod_locker
      }
    });
    const [output] = await sequelize.query('SELECT @p_locker_code AS locker_code, @p_success AS success, @p_message AS message');
...
export const getLockerPrice = async (req, res, next) => {
  ...
  if (!codLocker) {
    return res.status(400).json({
      message: 'Código del locker es requerido'
    });
  }
  try {
    ...
    res.status(500).json({
      ok: false,
      error: 'Error al obtener el precio del locker'
    });
    next();
  }
};