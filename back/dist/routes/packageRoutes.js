...
import { insertPackage, getPackageStatus, updatePackageStatus, updatePackageStatusLlegada, getPackageStatusEnBodega, getPackageStatusEnTransito, getPackageStatusListoParaRetirar, getPackageCounts } from '../controllers/PackageController.js';
...
router.post('/insert-package', upload.single('imagen'), async (req, res) => {
  try {
    ...
    if (!res.headersSent) {
      res.status(500).json({
        message: 'Error al insertar el paquete.'
      });
    }
  }
});
