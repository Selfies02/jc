...
export const signIn = async (req, res) => {
  ...
  try {
    const user = await sequelize.query(
      'SELECT * FROM SE_USERS WHERE EMAIL = ?',
    ...
    if (foundUser.EMAIL_VERIFIED === 0) {
      return res.status(403).json({ message: 'Correo no verificado. Por favor, verifica tu correo.' });
    }

    if (foundUser.IND_USR === 0) {
      return res.status(403).json({ message: 'Tu usuario está inactivo.' });
    }

    if (foundUser.IND_INS === 0) {
      return res.status(403).json({ message: 'Tu institución está inactiva.' });
    }
    ...
    const person = await PaPeople.findOne({
      where: { COD_PEOPLE: foundUser.COD_PEOPLE },
      include: [{ model: PaPhones }]
    });

    if (!person) {
      return res.status(404).json({ message: 'No se encontró la información de la persona asociada al usuario' });
    }

    const permissions = await sequelize.query(
      'CALL GET_ROL_OBJETOS_PERMISOS_FOR_USER(?)',
    ...
    if (roleName !== 'ADMINISTRADOR') {
      const customer = await sequelize.query(
        'SELECT COD_CUSTOMER FROM LO_CUSTOMERS WHERE COD_USER = ?',
        { replacements: [foundUser.COD_USER], type: sequelize.QueryTypes.SELECT }
      );

      if (!customer || customer.length === 0) {
        return res.status(404).json({ message: 'No se encontró el cliente asociado a este usuario' });
      }
    ...
    const token = jwt.sign(
      { userId: foundUser.COD_USER, roleId: foundUser.COD_ROL, customerId: foundCustomer },
