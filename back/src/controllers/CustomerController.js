...
export const getCustomerAddressAndPhone = async (req, res) => {
    ...
    try {
        const result = await sequelize.query(
            'CALL GET_CUSTOMER_ADDRESS_AND_PHONE(:cod_customer)', {
                replacements: { cod_customer },
                type: sequelize.QueryTypes.RAW
            }
        );
...
export const getCustomersWithStateAndType = async (req, res) => {
  try {
      ...
      if (!results || results.length === 0) {
          return res.status(202).json({ message: 'No hay clientes todavía' });
      }
...
export const updateCustomerStateAndType = async (req, res) => {
  const { codUser, newIndUsr, newIndTypCust, usrUpd } = req.body;

  try {
    const result = await sequelize.query(
      'CALL UPD_CUSTOMERS_WITH_STATE_AND_TYPE(:codUser, :newIndUsr, :newIndTypCust, :usrUpd)',
      {
        replacements: {
          codUser: codUser,
          newIndUsr: newIndUsr,
