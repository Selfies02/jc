...
const upload = multer({ dest: 'src/assets/facturas/' });

export const insertInvoiceAndCharges = [
  upload.single('pdfFile'),
  async (req, res) => {
    const invoiceData = JSON.parse(req.body.invoiceData);
    const { usr_add, charges } = invoiceData;
    ...
    try {
      ...
      const [results] = await sequelize.query(
        `CALL INS_INVOICE_AND_MULTIPLE_CHARGES(:usr_add, :charges, :pdfPath)`,
        {
          replacements: {
            usr_add,
      ...
      return res.status(500).json({ error: 'Ocurrió un error al insertar la factura y los cargos' });
    }
  }
];
...
export const getInvoiceTotalsByPeriod = async (req, res) => {
    ...
    if (period && !['day', 'month', 'year'].includes(period)) {
      return res.status(400).json({ ok: false, message: 'Parámetro de período inválido. Use "day", "month" o "year".' });
    }
  
    try {
      const result = await sequelize.query('CALL GET_INVOICE_TOTALS_BY_PERIOD(:period)', {
        replacements: { period: period || null }
      });
  
      const invoiceTotals = Array.isArray(result) ? result : [];
  
      if (invoiceTotals.length === 0) {
        return res.status(202).json({ ok: false, message: 'No hay datos de facturas disponibles todavía.' });
      }
  
      return res.status(200).json({ ok: true, invoiceTotals });
    } catch (error) {
