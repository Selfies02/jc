...
export const insertPackage = async (req, res) => {
  const { cod_customer, cod_locker, tracking } = req.body;
  ...
  try {
    const [result] = await sequelize.query(
      `CALL INS_PACKAGE(:cod_customer, :cod_locker, :tracking, :imagen)`, {
        replacements: { cod_customer, cod_locker, tracking, imagen }
      }
    );

    const [rows] = await sequelize.query(
      `SELECT p.FIRSTNAME, p.LASTNAME, e.EMAIL 
       FROM LO_CUSTOMERS c
    ...
    return res.status(200).json({ message: result.Resultado || 'Paquete insertado correctamente.' });
  } catch (error) {
    console.error('Error al insertar paquete:', error);
    return res.status(500).json({ message: 'Error al insertar paquete en tránsito.' });
  }
};
...
export const updatePackageStatus = async (req, res) => {
  const { cod_locker } = req.body;

  if (!cod_locker) {
    return res.status(400).json({ message: 'El campo cod_locker es requerido.' });
  }
  ...
  try {
    ...
    await sequelize.query(
      'CALL UPDATE_AND_NOTIFY_PACKAGE_STATUS(:cod_locker, @p_success, @p_message)',
      {
        replacements: { cod_locker }
      }
    );
    ...
    for (const packageInfo of packages) {
      const mailOptions = getPackageInTransitEmailTemplate(
        packageInfo.nombre,
        packageInfo.apellido,
        packageInfo.email,
        packageInfo.trackings,
        packageInfo.imagenes
      );
    ...
    return res.status(500).json({ message: 'Error al actualizar el estado de los paquetes.' });
  }
};
...
export const updatePackageStatusLlegada = async (req, res) => {
  ...
  try {
    ...
    await sequelize.query(
      'CALL UPDATE_AND_NOTIFY_PACKAGE_STATUS_LLEGADA(:cod_locker, @p_success, @p_message)',
      {
        replacements: { cod_locker }
      }
    );
    ...
    for (const packageInfo of packages) {
      const mailOptions = getPackageInTransitLlegada(
        packageInfo.nombre,
        packageInfo.apellido,
        packageInfo.email,
        packageInfo.trackings,
...
export const getPackageCounts = async (req, res) => {
  try {
    ...
    if (packageCounts.length === 0) {
      return res.status(202).json({ ok: false, message: 'No hay paquetes disponibles todavía.' });
    }

    return res.status(200).json({ ok: true, packageCounts });
  } catch (error) {
