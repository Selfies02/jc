...
export const verifyEmailController = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ cod_user, nombre }, process.env.JWTSECRET, { expiresIn: '15m' });
...
export const resetPasswordController = async (req, res) => {
  ...
  try {
    ...
    const result = await sequelize.query(
      'CALL UPD_USER_PASSWORD(:cod_user, :new_password, :usr_upd)',
      {
        replacements: { cod_user, new_password: hashedPassword, usr_upd },
    ...
    if (result && result[0] && result[0].message) {
      ...
      return res.status(500).json({ message: 'Error desconocido al actualizar la contraseña' });
    }
  } catch (error) {
