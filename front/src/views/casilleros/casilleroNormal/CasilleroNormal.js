...
const CasilleroNormal = () => {
  ...
  if (user?.permissions?.find((perm) => perm.object === 'CASILLERO NORMAL')?.canView === 0) {
  ...
  const filteredPackages = packages.filter((pkg) => pkg.COD_LOCKER === 2)

  return (
    <div>
      <h1 className="text-center">Paquetes en casillero normal</h1>
